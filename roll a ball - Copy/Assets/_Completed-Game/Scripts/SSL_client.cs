...
public class SSL_client : MonoBehaviour
{
    ...
    static string server = "192.168.1.25";
    //for google cloud
    ...
    private void Awake()
    {
        ...
        position = new Vector3(pos_x, 0.0f, pos_z);
    ...
    void FixedUpdate()
    {
        ...
        string pos_y = playerScript.transform.position.y.ToString();
        string pos_z = playerScript.transform.position.z.ToString();
        position= new Vector3(float.Parse(pos_x),float.Parse(pos_y),float.Parse(pos_z));
        Account user = new Account();
        if (float.Parse(pos_y) > -1.0f)
        {
